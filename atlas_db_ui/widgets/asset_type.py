...
from atlas_db.errors import DbAssetTypeAlreadyExistError
...
class AddAssetTypeDialog(qtw.QDialog):
...
class AssetTypeTableWidget(qtw.QWidget):
    """Base entity widget."""
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.setWindowTitle("Asset Type")

        self._view = qtw.QTableView(self)
        self._model = EntityTypeTableModel(AssetType)

        self._view.setModel(self._model)

        btn_add = qtw.QPushButton("Add Asset Type")
    ...
    def add_entity(self):
        ...
        if not code or not name:
            qtw.QMessageBox.critical(
                self,
                "Value Error",
                "Asset type code and name can't be None",
            )
            raise ValueError("Asset type code and name can't be None")

        # Ensure existing Asset type:
        with DbQueryContext() as db:
            asset_type = db.query(AssetType).where(AssetType.code == code).first()

        if asset_type is not None:
            msg = f"Asset type {code!r} already exists."
            qtw.QMessageBox.critical(
                self,
                "Asset Type already exists",
                msg,
            )
            raise DbAssetTypeAlreadyExistError(msg)

        with DbCommitContext() as db:
            new_entity = AssetType(code, name)
            db.add(new_entity)
